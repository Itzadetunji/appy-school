...
<?php
	...
	if ($current_password != "" && $new_password != "" && $confirm_new_password != "") {
		...
		if ($new_password == $confirm_new_password) {
			...
			$get_current_password_sql = "SELECT * FROM tbl_parent WHERE parent_system_code = '$parent_login_id'";
			...
			$stored_gotten_password = $get_current_password_row['parent_pswd'];

			if ($hashed_current_password == $stored_gotten_password) {
				
				$hashed_new_password = "c!a@l#e*b!".sha1(md5($new_password))."c!a@l#e*b!";

				$update_new_password_sql = "UPDATE tbl_parent SET parent_pswd = '$hashed_new_password' WHERE parent_system_code = '$parent_login_id'";
